...
export const getDeadlines = async (req: any, res: Response) => {
  try {
...
export const createDeadline = async (req: any, res: Response) => {
  try {
    const userId = req.user.id;
...
export const updateDeadlineStatus = async (req: any, res: Response) => {
  try {
    const userId = req.user.id;
    const { id } = req.params;
...
export const seedDefaultDeadlines = async (req: any, res: Response) => {
  try {
    const userId = req.user.id;

    // Kiểm tra xem đã có deadline nào chưa
    const existing = await prisma.deadline.findFirst({ where: { userId } });
    if (existing) {
