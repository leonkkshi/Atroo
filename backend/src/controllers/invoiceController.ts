...
export const analyzeInvoice = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const normalizedItems = (parsedData.items || []).map((item: any) => ({
    ...
    const calculatedSubtotal = normalizedItems.reduce((sum: number, item: any) => sum + (item.amount || 0), 0);
  ...
  try {
    ...
    const formattedInvoices = invoices.map((inv) => {
      ...
      try {
        ...
        const parsed: unknown = JSON.parse(inv.itemsJson);
...
function getMockInvoiceOcrJson(): string {
